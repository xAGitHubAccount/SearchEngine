...
public class RankedQueryParser {
	private TokenProcessor processor = new AdvTokenProcessor();
	...
	private static class Literal {
		StringBounds bounds;
		String literalComponent;
		
		Literal(StringBounds bounds, String literalComponent) {
			this.bounds = bounds;
	...
	public List<String> parseQuery(String query) {
		...
		do {
			...
			List<String> subqueryLiterals = new ArrayList<>(0);
			...
			// its component can go straight into the list.
                        for(String x: subqueryLiterals)
                        {
                            for(String y: processor.processTokenTwo(x))
                            {  
	...
	private Literal findNextLiteral(String subquery, int startIndex) {
		...
		while (subquery.charAt(startIndex) == ' ') {
			++startIndex;
                ...
                new String(subquery.substring(startIndex, startIndex + lengthOut)));
