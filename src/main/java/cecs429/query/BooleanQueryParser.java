	...
	private Literal findNextLiteral(String subquery, int startIndex) {
		...
		while (subquery.charAt(startIndex) == ' ') {
			++startIndex;
                ...
                {
                    // Locate the next quote to find the end of this literal.
                    int nextDouble = subquery.indexOf('\"', startIndex + 1);
                    int nextSpace = subquery.indexOf(' ', nextDouble);
                    ...
                    // This is a phrase literal containing a multiple terms.
                    return new Literal(new StringBounds(startIndex, lengthOut),
                    new PhraseLiteral((Arrays.asList(subquery.substring(startIndex, nextDouble).split(" ")))));
                }
                
                else if (subquery.charAt(startIndex) == '[')
                {
                    // Locate the next bracket to find the end of this literal.
                    int nextBracket = subquery.indexOf(']', startIndex + 1);
                    int nextSpace = subquery.indexOf(' ', nextBracket);
                            // No more literals in this subquery.
                        if (nextSpace < 0) {
                            // No more literals in this subquery.
                            lengthOut = subLength - startIndex;
                    ...
                    // This is a near literal containing a multiple terms.
                    return new Literal(new StringBounds(startIndex, lengthOut),
                    new NearLiteral((Arrays.asList(subquery.substring(startIndex, nextBracket).split(" ")))));
                }
