	...
	private Iterable<Path> findFiles() throws IOException {
		...
		Files.walkFileTree(mDirectoryPath, new SimpleFileVisitor<Path>() {
			...
			public FileVisitResult visitFile(Path file,
				...
				if (!mFactories.containsKey(extension)) 
                                {
                                    if(extension.equals(".json"))
                                    {
                                        registerFileDocumentFactory(extension,JsonFileDocument::loadJsonFileDocument);
                                        allFiles.add(file);
                                    }
                                    else if(extension.equals(".txt"))
                                    {
                                        registerFileDocumentFactory(extension,TextFileDocument::loadTextFileDocument);
                                        allFiles.add(file);
                                    }
				}
                                else
